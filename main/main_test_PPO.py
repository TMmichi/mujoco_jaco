...
if train:
    if separate:
        ...
        ent_coef = 'auto'
        if scratch:
            model = PPO2(MlpPolicy, env, tensorboard_log=save_path, verbose=1, policy_kwargs=policy_kwargs)
        else:
            load_policy_num = 3750000
            path = save_path+'/policy_'+str(load_policy_num)
            print("loaded_policy_path: ",path)
            #model = PPO2.load(path, env=env, learning_starts=10000, layers=layers, tensorboard_log=save_path)
