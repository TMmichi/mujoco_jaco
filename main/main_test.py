...
if train:
    if separate:
        ...
        model = SAC_MULTI.pretrainer_load(policy=MlpPolicy_sac, primitives=primitives, env=env, separate_value=True, tensorboard_log=tb_path)
...
if load:
    if separate:
        ...
        task = ['linear', 'angular']
        model = SAC_MULTI.load(model_path+"twowheel/"+task[0]+"2_SAC")
