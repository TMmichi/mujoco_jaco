...
class RL_controller:
    ...
    def train_from_scratch(self):
        ...
        task_list = metaworld.ML1.ENV_NAMES
        task = task_list[5]     # reach-v1
        prefix = task+"_trained_at_" + str(time.localtime().tm_mon) + "_" + str(time.localtime().tm_mday)\
        ...
        tasks = random.choice(ml1.train_tasks)
        env.set_task(tasks)
    ...
    def generate_traj(self):
        ...
        self.args.robot_file = "jaco2_curtain_torque"
        env = JacoMujocoEnv(**vars(self.args))
        traj_dict = generate_expert_traj(self._expert_3d, self.model_path+'/trajectories/'+self.args.task+'_trajectory_expert', env, n_episodes=100)
    ...
    def test(self):
        ...
        prefix = self.args.task + '_trained_at_12_10_15:26:52/policy_5100.zip'
        # prefix = self.args.task + '_trained_at_12_10_15:27:13/policy_4550.zip'

        model_dir = self.model_path + prefix
        test_iter = 100
        # self.model = SAC_MULTI.pretrainer_load(model_dir, MlpPolicy_sac, env)
        # self.model = PPO1.load(model_dir)
        self.model = SAC_MULTI.load(model_dir, policy=MlpPolicy_sac, env=env)
