...
class JacoMujocoEnvUtil:
    def __init__(self, **kwargs):
        ...
        self.gripper_angle_1, self.gripper_angle_2 = 0.5, 0.5
    ...
    def _reset(self):
        ...
        if self.task in ['carrying', 'releasing', 'placing']:
            ...
            quat = [0,0,0,0]
            if not self.task == 'reaching':
                self.interface.set_obj_xyz(self.obj_goal[0],quat)

        if self.task in ['grasping', 'carrying', 'pushing']:
            self.__get_gripper_pose()
    ...
    def _create_init_angle(self):
        if self.task in ['reaching', 'picking', 'pickAndplace']:
            angle0 = np.random.choice([uniform(0.8,1.2), uniform(1.8,2.2)])
            random_init_angle = [angle0, uniform(3.8,4), uniform(
                    1, 1.7), uniform(1.8, 2.5), uniform(1, 2.5), uniform(0.8, 2.3)]
            random_init_angle *= self.n_robots
        elif self.task in ['carrying', 'grasping', 'placing', 'pushing']:
