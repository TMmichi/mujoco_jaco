...
def ResidualBlock(x, out_dim, with_residual=True, with_batchnorm=True, trainable=True, name=None):
  conv1 = tf.layers.conv2d(
            inputs=x, filters=out_dim, kernel_size=3, padding='same',
            trainable=trainable, name=name+"/Conv1")
  if with_batchnorm:
    bn1 = tf.layers.batch_normalization(
            inputs=conv1, axis=-1, scale=True, training=trainable, name=name+"/BN1")
    relu1 = 
  #self.conv1 = nn.conv2d(in_dim, out_dim, kernel_size=3, padding=1)
  conv2 = tf.layers.conv2d(
            inputs=x, filters=out_dim, kernel_size=3, padding='same',
            activation=tf.nn.relu, trainable=trainable, name=name+"/Conv1")
